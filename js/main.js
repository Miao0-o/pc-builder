...
// ===================== 全局状态 =====================
const selectedBuild={};
const LS_KEY_BUILD='pcBuilder_currentBuild';

function _saveBuildToLS(){
  try{const clean={};for(const[k,v]of Object.entries(selectedBuild)){if(v)clean[k]=v;}
  localStorage.setItem(LS_KEY_BUILD,JSON.stringify(clean));}catch(e){}
}
function _restoreBuildFromLS(){
  try{const d=localStorage.getItem(LS_KEY_BUILD);if(d){const cfg=JSON.parse(d);
  for(const[k,n]of Object.entries(cfg)){if(pcComponents[k]){selectedBuild[k]=n;}}
...
const PART_OPTIONS = {
  case: [
    {id:'case-default',name:'ATX海景房',icon:'monitor',color:0x8a8a8a,preview:'银灰铝合金+钢化玻璃'},
  ],
  gpu: [
    {id:'gpu-dual',name:'双风扇显卡',icon:'gamepad-2',color:0x555555,preview:'中端双风扇·30cm级',size:[.1,.45,2.4]},
    {id:'gpu-triple',name:'三风扇显卡',icon:'gamepad-2',color:0x5a5a5a,preview:'高端三风扇·33cm级',size:[.1,.45,3.1]},
    {id:'gpu-flagship',name:'旗舰显卡',icon:'zap',color:0x4a4a4a,preview:'三槽旗舰·35cm级',size:[.14,.48,3.3]},
  ],
  motherboard: [
    {id:'mb-default',name:'ATX主板(黑)',icon:'layout-template',color:0x1a3a1a,preview:'标准ATX·黑色PCB'},
    {id:'mb-white',name:'ATX主板(白)',icon:'layout-template',color:0xcccccc,preview:'标准ATX·白色PCB',colorOverride:0xcccccc},
  ],
  fan: [
    {id:'fan-blue',name:'RGB风扇(蓝)',icon:'wind',emissive:0x3366ff,preview:'12cm·蓝色灯效'},
    {id:'fan-purple',name:'RGB风扇(紫)',icon:'wind',emissive:0x9933ff,preview:'12cm·紫色灯效'},
    {id:'fan-red',name:'RGB风扇(红)',icon:'wind',emissive:0xff3366,preview:'12cm·红色灯效'},
    {id:'fan-rainbow',name:'RGB风扇(彩虹)',icon:'palette',emissive:'rainbow',preview:'12cm·彩虹渐变'},
  ],
  cpu: [
    {id:'cpu-intel',name:'Intel CPU',icon:'cpu',color:0x555555,preview:'LGA1700'},
    {id:'cpu-amd',name:'AMD CPU',icon:'cpu',color:0x888888,preview:'AM5'},
  ],
  cooler: [
    {id:'cooler-air',name:'双塔风冷',icon:'fan',color:0x606060,preview:'双塔6热管'},
    {id:'cooler-aio',name:'240水冷',icon:'droplets',color:0x444444,preview:'240mm一体水冷'},
  ],
  ram: [
    {id:'ram-black',name:'DDR5内存(黑)',icon:'memory-stick',color:0x333333,preview:'16GB×2 无灯'},
    {id:'ram-rgb',name:'DDR5内存(RGB)',icon:'memory-stick',color:0x444444,preview:'16GB×2 RGB',rgb:true},
  ],
  storage: [
    {id:'ssd-default',name:'NVMe SSD',icon:'hard-drive',color:0x505050,preview:'M.2 2280'},
  ],
  psu: [
    {id:'psu-default',name:'ATX电源',icon:'plug-zap',color:0x5a5a5a,preview:'750W金牌全模组'},
  ],
};
...
// ===================== 聚焦系统 =====================
function _focusOnObject(key){let grpKey=key;if(key==='fan'&&componentGroups['fan-front-0'])grpKey='fan-front-0';const group=componentGroups[grpKey];if(!group)return;for(const[k,g]of Object.entries(componentGroups)){const match=(k===grpKey||(key==='fan'&&k.startsWith('fan-')));_setGroupStyle(g,match?'real':'deep-ghost');}selectedPart=key;_updateLabels();const bb=new THREE.Box3().setFromObject(group);const center=new THREE.Vector3();bb.getCenter(center);const sT=controls.target.clone(),sC=camera.position.clone();const dir=sC.clone().sub(sT).normalize();const sz=new THREE.Vector3();bb.getSize(sz);const dist=Math.max(Math.max(sz.x,sz.y,sz.z)*1.5,4);const eT=center.clone(),eC=center.clone().add(dir.multiplyScalar(dist));const t0=performance.now();function anim(now){const t=Math.min((now-t0)/600,1),e=1-Math.pow(1-t,3);controls.target.lerpVectors(sT,eT,e);camera.position.lerpVectors(sC,eC,e);controls.update();if(t<1)requestAnimationFrame(anim);}requestAnimationFrame(anim);const lookupKey=(key.startsWith('fan-')||key==='fan')?'fan':key;if(pcComponents[lookupKey])_showDetail(lookupKey);else _showGeneric(key);if(document.getElementById('tabBuy')){currentTab='buy';document.getElementById('tabBuy').classList.add('active');document.getElementById('tabIntro')?.classList.remove('active');_renderTabContent();if(typeof lucide!=='undefined')lucide.createIcons();}document.getElementById('btnResetView')?.classList.add('visible');}
...
function _focusOnComponent(key){
  ...
  // Switch to in-store tab after detail renders
...
function _loadPreselected(){try{const d=localStorage.getItem('pcBuilderPreselect');if(d){const cfg=JSON.parse(d);for(const[k,n]of Object.entries(cfg)){if(pcComponents[k]){selectedBuild[k]=n;}}localStorage.removeItem('pcBuilderPreselect');_renderSummary();if(componentGroups['cpu'])_focusOnObject('cpu');return;}if(window.location.hash){const m=window.location.hash.match(/^#config\/([A-Z0-9]{6})$/);if(m){const sd=localStorage.getItem('pcBuilder_sharedConfig_'+m[1]);if(sd){const sc=JSON.parse(sd);for(const[k,n]of Object.entries(sc.config)){if(pcComponents[k])selectedBuild[k]=n;}_renderSummary();}}}}catch(e){}}
function initServicePage(){document.querySelectorAll('.service-card').forEach((c,i)=>{c.style.animationDelay=`${.2+i*.12}s`;});}

if(document.getElementById('case3D')){initParticles();init3DScene();_restoreBuildFromLS();_renderSummary();_loadPreselected();}
