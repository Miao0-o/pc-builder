...
// ===================== 聚焦系统 =====================
function _focusOnObject(key){const group=componentGroups[key];if(!group)return;for(const[k,g]of Object.entries(componentGroups))_setGroupStyle(g,k===key?'real':'deep-ghost');selectedPart=key;_updateLabels();const bb=new THREE.Box3().setFromObject(group);const center=new THREE.Vector3();bb.getCenter(center);const sT=controls.target.clone(),sC=camera.position.clone();const dir=sC.clone().sub(sT).normalize();const eT=center.clone(),eC=center.clone().add(dir.multiplyScalar(4));const t0=performance.now();function anim(now){const t=Math.min((now-t0)/600,1),e=1-Math.pow(1-t,3);controls.target.lerpVectors(sT,eT,e);camera.position.lerpVectors(sC,eC,e);controls.update();if(t<1)requestAnimationFrame(anim);}requestAnimationFrame(anim);if(pcComponents[key])_showDetail(key);else _showGeneric(key);document.getElementById('btnResetView')?.classList.add('visible');}
